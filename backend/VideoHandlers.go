...
func GetUserVideoHandler(ctx *fasthttp.RequestCtx) {
	...
	if err := Postgres.Conn.QueryRow(context.Background(), query, userId, limit, offset).Scan(&VideoStruct.UserVideos); err != nil {
		fmt.Println(err)
		return
	}
...
func PostVideoHandler(ctx *fasthttp.RequestCtx) {
	...
	if err := Postgres.Conn.QueryRow(context.Background(), query, adderId, title).Scan(&videoId); err != nil {
		fmt.Println(err)
		return
	}
	...
	if _, err := Postgres.Conn.Exec(context.Background(), query, videoId, adderId); err != nil {
		fmt.Println("err??", err)
		return
	}
...
func DeleteVideoHandler(ctx *fasthttp.RequestCtx) {
	...
	if _, err := Postgres.Conn.Exec(context.Background(), query, videoId, userId); err != nil {
