...
func GetUserMusicHandler(ctx *fasthttp.RequestCtx) {
	...
	if err := Postgres.Conn.QueryRow(context.Background(), query, userId, limit, offset).Scan(&AudioStruct.UserMusic); err != nil {
		fmt.Println(err)
		return
	}
...
func GetCombinedMusicHandler(ctx *fasthttp.RequestCtx) {
	...
	if offset == "0" {
		...
		if err := Postgres.Conn.QueryRow(context.Background(), query, userId, withVal).Scan(&AudioStruct.UserMusic); err != nil {
			fmt.Println(err)
			ctx.SetStatusCode(400)
			return
		}
	...
	if err := Postgres.Conn.QueryRow(context.Background(), query, withVal, limit, offset).Scan(&AudioStruct.AllMusic); err != nil {
		fmt.Println(err)
		ctx.SetStatusCode(400)
		return
	}
...
func PostMusicHandler(ctx *fasthttp.RequestCtx) {
	...
	if err := Postgres.Conn.QueryRow(context.Background(), query, adderId, title, author).Scan(&musicId); err != nil {
		fmt.Println(err)
		return
	}
	...
	if _, err := Postgres.Conn.Exec(context.Background(), query, musicId, adderId); err != nil {
		fmt.Println(err)
		return
	}
...
func DeleteMusicHandler(ctx *fasthttp.RequestCtx) {
	...
	if _, err := Postgres.Conn.Exec(context.Background(), query, musicId, userId); err != nil {
